...
export const recuperationTradeParFiltre = async (tableauFiltreValue, collectionValues) => {
    ...
    if (tableauFiltreValue !== null && tableauFiltreValue !== undefined && tableauFiltreValue !== '' && tableauFiltreValue.length !== 0) {
        if (collectionValues !== null && collectionValues !== undefined && collectionValues !== '') {
            ...
            tableauFiltreValue.forEach(objet => {
                filtreDeBase = objet.filtreDeBase;
                console.log(filtreDeBase);
            });

            tableauFiltreValue.shift();
            console.log(tableauFiltreValue);
            console.log(tableauFiltreValue.length);

            if (filtreDeBase !== null && filtreDeBase !== undefined && filtreDeBase !== '') {
                console.log("Vérification filtreDeBase validée");
                if (tableauFiltreValue !== null && tableauFiltreValue !== undefined) {
                    console.log("Vérification tableau validée");
                    try {
                        const response = await axios.get(`${apiUrl}/${filtreDeBase}`, {
                            params: {
                                collection: collectionValues,
                                username: username,
                                filtreDeBase: filtreDeBase,
                                tableauFiltreValue: JSON.stringify(tableauFiltreValue)
                            }
                        });
